  ...
  <div class="min-h-screen w-full flex flex-col items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-[400px] relative z-10 animate-fade-in-up">
      <div class="mb-10 text-center">
        <h2 class="font-display font-medium text-3xl tracking-wide uppercase text-white mb-2">Iniciar sesión</h2>
        <p class="text-textMuted uppercase tracking-widest text-xs font-light">Acceso para clientes</p>
      </div>

      <form @submit.prevent="handleLogin" class="space-y-6 bg-black/40 p-8 border border-white/5 shadow-2xl backdrop-blur-sm">
        ...
        <div class="mt-6 text-center text-xs font-light text-textMuted">
          ¿No tienes cuenta? 
          <router-link :to="{ path: '/client/register', query: { redirect: route.query.redirect } }" class="text-primary hover:text-white transition-colors border-b border-primary/30 pb-0.5">
            Regístrate aquí
          </router-link>
        </div>
      </form>
